// Общий кэш списков в localStorage.
//
// Зачем: бэкенд на Apps Script отвечает 5–8 секунд (замерено), и без кэша
// каждое переключение вкладки означало бы это ожидание заново.
//
// Правило простое: показываем то, что есть, и на сервер идём только когда
// кэша нет, он старше FRESH_MS или человек сам нажал «Обновить». Раньше
// каталог показывал кэш и всё равно молча шёл на сервер — ожидание никуда
// не девалось, просто становилось незаметным.
//
// Возраст данных экраны обязаны показывать. На складе это не косметика: если
// «Доступно» нарисовано по двадцатиминутным данным, человек пойдёт за
// техникой, которую уже выдали.
