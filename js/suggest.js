...
//
// Поиск раньше молчал, пока не наберёшь достаточно букв, и человек должен был
// помнить, что вообще есть на складе. А названия там латиницей и капсом —
// OSTERRIG SIRIUS 100CM, GODOX OCTABOX 120 — их набирают по памяти и с
// ошибками. Подсказки показывают, что существует, ещё до того, как название
// набрано целиком.
//
// Ни одного запроса к таблице это не стоит: все три экрана уже фильтруют
// локально по данным из кэша, и подсказки считаются по тем же данным.
//
// Выбор подсказки только подставляет текст в строку поиска и рассылает обычное
// событие input — экран фильтруется своим прежним обработчиком, и логику
// фильтрации ни на одном экране трогать не нужно.
